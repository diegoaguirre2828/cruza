...
// 2026-05-03 sister-site refactor: iOS-app users see ONLY the Free tier so
// Apple Review can never trigger 2.1(b) IAP error or 3.1.1 external-payment
// rejection. All B2B / Stripe paid tiers (Pro/Business/Operator/Express
// Cert/Intelligence/Enterprise) are filtered out of the iOS plans grid +
// the surrounding sales/cost-impact/advertise sections are also iOS-hidden.
// `isIosApp` is detected SERVER-SIDE in app/pricing/page.tsx via
// isIOSAppServer() and passed in as a prop, so the SSR HTML never contains
// a Stripe button on iOS UA — no flash, no race for Apple's review tools.
// Once Diego completes the Paid Apps Agreement + IAP attach, flip the
// filter to also include 'pro' (IOSSubscribeButton already handles the
// IAP path correctly).
export function PricingClient({ isIosApp }: { isIosApp: boolean }) {
  const { user } = useAuth()
    ...
    <main className="min-h-screen bg-gray-50 dark:bg-gray-950">
      <div className="max-w-4xl mx-auto px-4 pb-16">
        ...
        {/* Enterprise sales-inquiry capture form (anchored from
            ...
            iOS-hidden: form is for B2B Enterprise tier which isn't
            shown on iOS, and the form's Stripe-adjacent framing risks
            a 3.1.1 review flag. */}
        {!isIosApp && <SalesInquiryForm es={es} />}

        {/* Business cost impact — B2B fleet pitch. iOS-hidden so the
            consumer-only iOS view doesn't surface the Business tier
            value-prop section that references tiers we no longer
            render here. */}
        {!isIosApp && (
        ...
        {/* Business advertise CTA — iOS-hidden (B2B-leaning surface). */}
        {!isIosApp && (
        ...
        {/* iOS-only — gentle pointer to the web for advanced features.
            Shown after the Free tier card so iOS reviewers see a clean
            consumer-only view but power users still know where to go
            for Pro/Business/Operator/Intelligence. */}
        {isIosApp && (
          <div className="mt-6 bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-2xl p-5 text-center">
