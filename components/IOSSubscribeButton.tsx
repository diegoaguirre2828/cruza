...
// iOS-only subscribe button for Pro. Shown in place of the Stripe
// checkout button on the pricing page when the client is a Capacitor
// iOS build (Apple 3.1.1 — StoreKit only for digital subs).
//
// Backend flow:
//   RevenueCat purchasePackage → StoreKit sheet → entitlement `pro`
//   active → customer info posted to our backend via RevenueCat's
//   App Store Connect webhook (configured in RevenueCat dashboard).
...
export function IOSSubscribeButton({
  ...
  return (
    <div className={className}>
      ...
      >
        {restoring
          ? (es ? 'Restaurando…' : 'Restoring…')
          : (es ? 'Restaurar compra' : 'Restore purchase')}
      </button>
      {error && (
