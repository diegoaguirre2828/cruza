...
export function PhoneButton({ label }: { label?: string }) {
  ...
  const { lang } = useLang()
  const es = lang === 'es'
  const effectiveLabel = label ?? (es ? 'Continuar con teléfono' : 'Continue with Phone')
      ...
      <form onSubmit={sendOtp} className="space-y-3">
        ...
        <div>
          ...
          <p className="text-xs text-gray-400 mt-1">
            {es
              ? 'Incluye código de país: +1 pa\' US, +52 pa\' México'
              : 'Include country code: +1 for US, +52 for Mexico'}
          </p>
        </div>
    ...
    return (
      <form onSubmit={verifyOtp} className="space-y-3">
        <p className="text-xs text-gray-500 text-center">
          {es
            ? <>Te mandamos un código de 6 dígitos a <strong className="text-gray-800">{phone}</strong></>
        ...
        <div>
          <label className="block text-xs font-medium text-gray-700 mb-1">
            {es ? 'Código de verificación' : 'Verification code'}
          </label>
          <input
            type="text"
            inputMode="numeric"
  ...
  return (
    <div className="w-full flex items-center justify-center py-2.5 text-sm text-gray-500">
      ...
      {otp
        ...
        : (es ? 'Enviando código...' : 'Sending code...')}
    </div>
  )
}
