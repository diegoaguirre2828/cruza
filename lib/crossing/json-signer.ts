// lib/crossing/json-signer.ts
//
// Ed25519 signing for Cruzar Crossing artifacts. Reuses the same signing
// key as Cruzar Ticket (CRUZAR_TICKET_SIGNING_KEY env) — one trust root
// for all signed Cruzar artifacts. The signed payload type differs
// (Crossing vs Ticket), but the signature scheme + canonicalization +
// hash is identical, so a single public key verifies all artifacts.

import * as ed from '@noble/ed25519';
import { createHash } from 'crypto';
import type { CruzarCrossingV1, SignedCrossing } from './types';
import { canonicalize } from '../ticket/json-signer';
...
export async function signCrossing(payload: CruzarCrossingV1): Promise<SignedCrossing> {
...
export async function verifyCrossing(signed: SignedCrossing, publicKeyB64?: string): Promise<{ valid: boolean; reason?: string }> {
