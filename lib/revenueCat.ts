...
// `app.cruzar.ios.pro.monthly` ($2.99/mo w/ 7-day free trial).
//
// Web, Android, and non-app iOS Safari users continue to use the Stripe
// flow untouched. This module is a no-op on those platforms.
...
export async function purchaseProMonthly(): Promise<{ purchased: boolean; error?: string }> {
  ...
  try {
    ...
    // User-cancelled purchase surfaces as an error but is not a failure
    // state to show — swallow silently so the UI just re-enables.
